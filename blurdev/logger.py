...
def setStreamHandlerStream(handler, stream):
...
def replaceStreamForStreamHandlers(old, new, logger=None):
    """Replaces the stream of StreamHandlers by checking all `logging.StreamHandler`'s
    attached to the provided logger. If any of them are using old for their stream,
    update that stream to new.
    ...
    Args:
        ...
        logger (logging.Logger, optional): The logger to update streams for. If None,
            the root logger(`logging.getLogger()`) will be used.
    """
    if logger is None:
    ...
    for handler in logger.handlers:
        if isinstance(handler, logging.StreamHandler):
            if handler.stream == old:
                setStreamHandlerStream(handler, new)
