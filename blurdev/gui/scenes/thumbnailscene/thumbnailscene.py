...
class ThumbnailScene(QGraphicsScene):
            ...
            if selection:
                ...
                if item in items:
                    ...
                    if index > 0:
                        self.blockSignals(
                            True
                        )  # don't emit the selectionChanged signal twice
                        self.clearSelection()
    ...
    def recalculate(self, rect, force=False):
        ...
        if self.layoutDirection() == Qt.Vertical and not (
            force or rect.width() != self.lastRect().width()
        ):
            return True
        elif self.layoutDirection() == Qt.Horizontal and not (
            force or rect.height() != self.lastRect().height()
        ):
            return True
