...
class XMLElement:
    ...
    def children(self):
        ...
        if self._object:
            return [
                XMLElement(child, self.__file__)
                for child in self._object.childNodes
                if isinstance(child, xml.dom.minidom.Element)
            ]
        return []
    ...
    def findChildById(self, key):
        ...
        for child in self.children():
            if (
                key == child.getId()
                or key == '_'.join(re.findall('[a-zA-Z0-9]*', child.nodeName)).lower()
            ):
                return child
        return None
    ...
    def findChildren(self, childName, recursive=False):
        ...
        if self._object:
            if recursive:
                return [
                    XMLElement(child, self.__file__)
                    for child in self._object.getElementsByTagName(childName)
                ]
            else:
                return [
                    XMLElement(child, self.__file__)
    ...
    def findColor(self, name, fail=None):
        ...
        if element:
            return QColor(
                float(element.attribute('red')),
                float(element.attribute('green')),
                float(element.attribute('blue')),
                float(element.attribute('alpha')),
            )
        elif fail:
            return fail
        else:
