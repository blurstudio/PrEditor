...
if ( __name__ == '__main__' ):
    ...
    if ( dictionary['offline'] == '1' ):
        ...
        if trax.isValid:
            ...
            blurdev.traxoffline.trax.api.data.createRoleList( r'%s\traxoffline\trax\api\data\source\roles.xml' % path )
            blurdev.traxoffline.trax.api.data.createApprovalType( r'%s\traxoffline\trax\api\data\source\approvalTypes.xml' % path )
            blurdev.traxoffline.trax.api.data.createDepartmentList( r'%s\traxoffline\trax\api\data\source\departments.xml' % path )
    ...
    if ( results ):
        ...
        if ( dictionary['offline'] == '1' ):
            outFile = '%s-install-v%i.%02i.%s-offline.exe' % (product,version.major(),version.minor(),results.groups()[0])
            filename = path + '/installers/bin/offline/' + outFile
        else:
            outFile = '%s-install-v%i.%02i.%s.exe' % (product,version.major(),version.minor(),results.groups()[0])
