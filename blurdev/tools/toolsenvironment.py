...
class ToolsEnvironment(QObject):
    ...
    def clearPathSymbols(self):
        ...
        pythonpath = [split.lower() for split in os.environ.get('pythonpath', '').split(';')]
        ...
        oldpaths = sys.path
        newpaths = [spath for spath in sys.path if (path not in spath.lower() and spath != '.') or spath.lower() in pythonpath]
        sys.path = newpaths
    ...
    def timeoutTimedelta(self):
        if not self._timeout:
            return None
        try:
            type_map = {'d': 'days', 'm': 'minutes', 'h': 'hours', 's': 'seconds', 'w': 'weeks'}
            d = {}
    ...
    def setEmailOnError(self, emails):
        if not emails:
            ...
            self._emailOnError = [ entry for entry in emails if str(entry) != '' and entry != None ]
    ...
    def syncINIEnvironment(env):
        ...
        if os.path.exists(codeRootPath):
            blurdev.ini.SetINISetting(blurdev.ini.configFile, legacy, 'codeRoot', codeRootPath)
            blurdev.ini.SetINISetting(blurdev.ini.configFile, legacy, 'startupPath', os.path.abspath(os.path.join(envPath, 'maxscript', 'treegrunt', 'lib')))
    ...
    @staticmethod
    def createNewEnvironment(name, path, default=False, development=False, offline=True, environmentFile='', legacyName=None):
        """
    ...
    def loadConfig(filename, included=False):
        ...
        if (doc.load(filename)):
            ...
            if (not included):
                pref = blurdev.prefs.find('blurdev/core', coreName=blurdev.core.objectName())
...
# 						ToolsEnvironment.defaultEnvironment().setActive(silent=True)
                ...
                found = False
                if environPath:
                    env = ToolsEnvironment.findEnvironment(blurdev.tools.TEMPORARY_TOOLS_ENV)
                    if env.isEmpty():
                        found = True
                if not found:
                    ToolsEnvironment.defaultEnvironment().setActive(silent=True)
    ...
    def registerPaths(self):
        ...
        self.registerPath(os.path.join(blurdev.activeEnvironment().path(), 'code', 'python', 'tools'))
        self.registerPath(os.path.join(blurdev.activeEnvironment().path(), 'maxscript', 'treegrunt', 'lib'))
        # make environment libs importable
        self.registerPath(os.path.join(blurdev.activeEnvironment().path(), 'code', 'python', 'lib'))
