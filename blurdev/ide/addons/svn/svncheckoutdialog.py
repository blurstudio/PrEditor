...
class SvnCheckoutDialog( Dialog ):
    ...
    def accept( self ):
        ...
        if ( not (self.url() and self.filepath()) ):
            msg.critical(self,'Missing Data', 'You need to provide a URL and filepath for the checkout to work.' )
            return False
    ...
    def pickPath( self ):
        filepath = QFileDialog.getExistingDirectory( self, 'Checkout Directory', self.filePath() )
        if ( filepath ):
