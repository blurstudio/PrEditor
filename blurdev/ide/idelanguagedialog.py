...
from Qt.QtWidgets import QComboBox, QDialog, QDialog, QDialogButtonBox, QHBoxLayout, QInputDialog, QLineEdit \
...
class DescriptorDialog( QDialog ):
    def __init__( self, parent ):
        ...
        self.uiDialogBTNS   = QDialogButtonBox( QDialogButtonBox.Ok | QDialogButtonBox.Cancel, Qt.Horizontal, self )
    ...
    def accept( self ):
        ...
        if ( not (type and expr) ):
            QMessageBox.critical( self, 'Not Enough Info', 'You need to select a type and enter an expression.')
            return
...
class IdeLanguageDialog( QDialog ):
    def __init__( self, parent = None ):
        ...
        if ( scheme ):
            ...
            for key in scheme.properties():
                if ( key.startswith('document_color_') ):
                    ...
                    item.setBackground( 0, self.palette().color(self.palette().AlternateBase))
        ...
        self.uiRemoveDescriptorBTN.setIcon(     QIcon(blurdev.resourcePath('img/remove.png')))
        ...
        self.uiLexerClassDDL.currentIndexChanged.connect(    self.refreshSchemeItemsFromCurrent )
    ...
    def editDescriptor( self ):
        ...
        if ( not item ):
            QMessageBox.critical( self, 'No Item Selected', 'You have no descriptors selected to edit.' )
            return False
    ...
    def handleDropEvent( object, event ):
        ...
        if ( not dragItem ):
            ...
            QMessageBox.critical( self, 'Cannot Move', 'You cannot move the color items.' )
            return
        ...
        if ( targetItem ):
            ...
            if ( dragItem.parent() ):
                ...
                dragItem.treeWidget().takeTopLevelItem(dragItem.treeWidget().indexOfTopLevelItem(dragItem))
    ...
    def recordUi( self, language ):
        ...
        for i in range(self.uiEditorColorsTREE.topLevelItemCount()):
            ...
            lexerColorTypes[str(item.text(0))] = [item.child(c).data(0,Qt.UserRole) for c in range(item.childCount())]
    ...
    def refreshUi( self, language ):
        ...
        index = self.uiLexerClassDDL.findText(language.lexerClassName().replace('QsciLexer',''))
        if ( index != -1 ):
    ...
    def removeDescriptor( self ):
        item = self.uiDescriptorTREE.currentItem()
        if ( item and QMessageBox.question( self, 'Remove Item', 'Are you sure you want to remove this descriptor?', QMessageBox.Yes | QMessageBox.No ) == QMessageBox.Yes ):
            self.uiDescriptorTREE.takeTopLevelItem(self.uiDescriptorTREE.indexOfTopLevelItem(item))
    
    def save( self, filename = '' ):
        if ( not self.uiNameTXT.text() ):
            QMessageBox.critical( self, 'No Name Supplied', 'You need to provide a name for this language.' )
            return False
    ...
    def loadFrom( self ):
        langname, accepted = QInputDialog.getItem( self, 'Select Language', 'Load from language:', lang.languages() )
        if ( accepted ):
